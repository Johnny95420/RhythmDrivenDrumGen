...
from model.layers import BetaLatentMultiDim, Encoder, EncoderInputAdapter, OutputAdapter
...
if __name__ == "__main__":
    ...
    model = CausalDecoderWrapper(
        ...
        intermediate=BetaLatentMultiDim(32, 512, 0.25).cuda(),
        decoder=Phi3GrooveModel(default_config, groove_adapter_paras),
        output_head=OutputAdapter(128, 9),
    ).cuda()
    ckpt = torch.load("/home/vae_causal_ckpt/ckpt_24500.ckpt")
    ...
    z, vae_loss = model.encode(
        data["onset_reduce"][:, :33],
        data["vel_reduce"][:, :33],
        data["timeoffset_reduce"][:, :33],
        data["masks"][:, :33],
    )

    input_ids, condition, _ = model.process_decoder_input(
        data["onset"][:, :1, :],
        data["vel"][:, :1, :],
        data["timeoffset"][:, :1, :],
        z,
    ...
    with torch.autocast("cuda", torch.bfloat16):
        for i in range(64):
            output = model.decoder(input_ids, condition)
            output = model.output_head(output.last_hidden_state)
            h = 1.0 * ((output[0][:, -1, :].sigmoid()) >= 0.5)
    ...
    # %%
    file_path = "/home/test_clip/Ostinato Dueling Dragonflies Eb Major 110 bpm.mid"
    piano_roll_onset, piano_roll_vel, piano_roll_timeoffset = process_midi(file_path)
    # piano_roll_onset = torch.cat(
    #     [piano_roll_onset, torch.zeros([1, 4, 9], device="cuda"), piano_roll_onset],
    #     dim=1,
    # )
    ...
    #     dim=1,
    # )
    onset_reduce, vel_reduce, timeoffset_reduce = (
        piano_roll_onset[:, :33],
        piano_roll_vel[:, :33],
        piano_roll_timeoffset[:, :33],
    )
    density, intensity, style, inst = assign_generate_paramter(
        -1, -1, 2, [0.2, 0.1, 0.0, 0.2, 0.0, 0.0, 0, 0, 0.2], "cuda"
    )
    model.eval()
    z, vae_loss = model.encode(
        onset_reduce, vel_reduce, timeoffset_reduce, data["masks"]
    )
    input_ids, condition, _ = model.process_decoder_input(
        data["onset"][:, :1, :],
        data["vel"][:, :1, :],
